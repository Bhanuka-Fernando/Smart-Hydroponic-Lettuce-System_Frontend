...
type Props = NativeStackScreenProps<WaterQualityStackParamList, "WaterStatus">;
type Status = "OK" | "WARNING" | "CRITICAL" | "—";

const THEME = "#00368C";

function statusTheme(status: Status) {
  if (status === "OK") return { bg: "#EAF9F0", border: "#BBF7D0", text: "#047857", solid: "#16A34A" };
  if (status === "WARNING") return { bg: "#FFF3D6", border: "#F9C74F", text: "#7A3E00", solid: "#D97706" };
  if (status === "CRITICAL") return { bg: "#FFECEE", border: "#FECACA", text: "#B42318", solid: "#EF4444" };
  return { bg: "#F3F4F6", border: "#E5E7EB", text: "#374151", solid: THEME };
}
...
export default function WaterStatusScreen({ navigation, route }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setErr(e?.response?.data?.detail ?? e?.message ?? "Failed to load water details.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const theme = statusTheme(finalStatus);
  const score = Math.max(0, Math.min(100, result?.health_score ?? 0));
      ...
      <ScrollView className="flex-1 px-5" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        <View className="pt-2 pb-4 flex-row items-center justify-between">
          ...
          <View className="items-center">
            <Text className="text-[19px] font-extrabold text-gray-900">Water Status</Text>
        ...
        {loading ? (
          ...
          <>
            <View className="rounded-[26px] border p-5 mb-4 shadow-sm" style={{ backgroundColor: theme.bg, borderColor: theme.border }}>
              <Text className="text-[12px] font-extrabold uppercase" style={{ color: theme.text }}>Final status</Text>
              <Text className="mt-2 text-[30px] font-extrabold" style={{ color: theme.text }}>{finalStatus}</Text>
              <Text className="mt-2 text-[14px] font-bold text-gray-700">Health score: {score}/100</Text>
              <Text className="mt-1 text-[12px] font-semibold text-gray-600">Rule status: {result.rule_status}</Text>
            </View>

            <Section title="Main guidance" icon="sparkles">
              <Text className="text-[12px] font-bold text-gray-500">Main reason</Text>
              <Text className="mt-1 text-[14px] leading-5 font-extrabold text-gray-900">{result.main_reason || "No main reason available."}</Text>
              <Text className="mt-4 text-[12px] font-bold text-gray-500">Main action</Text>
              <Text className="mt-1 text-[14px] leading-5 font-extrabold text-gray-900">{result.main_action || "No action needed."}</Text>
            </Section>

            <Section title="Water confidence" icon="shield-checkmark">
              <ProbBar label="OK" value={result.ml_probs.OK ?? 0} color="#16A34A" />
              <ProbBar label="WARNING" value={result.ml_probs.WARNING ?? 0} color="#F59E0B" />
              <ProbBar label="CRITICAL" value={result.ml_probs.CRITICAL ?? 0} color="#EF4444" />
            </Section>

            <Section title="Reasons" icon="list">
              {result.reasons?.length ? result.reasons.map((r, idx) => (
                <ListItem key={idx} text={r} icon="information-circle" color={theme.solid} />
              )) : <Text className="text-[13px] text-gray-600">No water issues detected.</Text>}
            </Section>

            <Section title="Actions" icon="checkmark-done">
              {result.actions?.length ? result.actions.map((a, idx) => (
